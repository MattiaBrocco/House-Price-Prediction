...
class Cleaners:
    ...
    def house_price_cleaner(self, df):
        ...
        df = df.drop(["EASE-MENT","Unnamed: 0"], axis = 1)
        ...
        df["sale_price"] = df['sale_price'].replace(' -  ' , 0) 
        
        # Replace given string with nan
        df['tax_class_at_present'] = df['tax_class_at_present'].replace(" " , df['tax_class_at_present'].mode())
        df['building_class_at_present'] = df['building_class_at_present'].replace(" " , df['building_class_at_present'].mode())
        df['year_built'] = df['year_built'].replace(0 , int(df['year_built'].mean()))
